...
def plotAbundancesSimple(abundances):
    ...
    cmap='jet'
    if not os.path.exists("./urban_abundance"):
        os.makedirs("./urban_abundance")
    ...
    for i in range(num_endmembers):
        ...
        plt.tight_layout()
        # save_path = os.path.join("./urban_abundance", f'{"abundance"}{i}.svg')  # 使用f-string格式化文件名
        # plt.savefig(save_path, format='svg')  # 保存为SVG格式
        plt.show()
        # plt.savefig(f"{'./result_p/VCA'}_endmember_{i + 1}.png")
        plt.close()

data = sio.loadmat("./Results/0_SSABN/perfect result/Sy20_data_run1.mat")
data_t = sio.loadmat("./Datasets/Datasets/sy20.mat")
