...
class CMSIB(Layer):
    def __init__(self):
        super(CMSIB, self).__init__()
    def call(self, inputs):
        b,h,w,c = inputs.shape
        scale11 = Conv2D(filters=c,kernel_size=3,groups=2,padding='same',strides=1)(inputs)
        scale11 = ReLU()(scale11)
        scale12 = Conv2D(filters=c,kernel_size=3,strides=1,padding='same')(inputs)
        scale12 = ReLU()(scale12)
        scale1 = concatenate([scale11,scale12],axis=3)
        scale21 = Conv2D(filters=2*c,kernel_size=3,groups=2,padding='same',strides=1)(scale1)
        scale21 = ReLU()(scale21)
        scale22 = Conv2D(filters=2*c,kernel_size=3,padding='same',strides=1)(scale1)
        scale22 = ReLU()(scale22)
        scale2 = concatenate([scale21,scale22],axis=3)
        output = Conv2D(filters=c,kernel_size=1)(scale2)
        return output
...
class Encoder(Model):
    ...
    def call(self, input_patch):
        ...
        # Feature0 = Conv2D(filters=c,kernel_size=3,padding='same',strides=1)(input_patch1)
        # Feature = Feature0
        # f_l = [None for _ in range(16)]
        # f_l[0] = Feature0
        # for i in range(15):
        #     f_l[i+1] = self.CMSIB(f_l[i])
        #     Feature = concatenate([Feature,f_l[i+1]])
        #
        # B,H,W,C = Feature.shape
        # w_2d = GlobalAvgPool2D()(Feature)
        # w_b,features = w_2d.shape
        # w_4d = tf.reshape(w_2d,[-1,1,1,features])
        # w_4d = Conv2D(filters=C//reduction,kernel_size=1,strides=1,padding='valid',use_bias=True)(w_4d)
        # w_4d = ReLU()(w_4d)
        # w_4d = Conv2D(filters = C, kernel_size=1,strides=(1,1),padding='valid',use_bias=True)(w_4d)
        # w_4d = tf.nn.sigmoid(w_4d)
        # w_2d = tf.reshape(w_4d,[-1,C])
        # code2 = KL.multiply([w_2d, Feature])



        # B,H,W,C = code2.shape
        # w_2d = GlobalAvgPool2D()(code2)
        # w_b,features = w_2d.shape
        # w_4d = tf.reshape(w_2d,[-1,1,1,features])
        # w_4d = Conv2D(filters=C//reduction,kernel_size=1,strides=1,padding='valid',use_bias=True)(w_4d)
        # w_4d = ReLU()(w_4d)
        ...
        # w_4d = tf.nn.sigmoid(w_4d)
        # w_2d = tf.reshape(w_4d,[-1,C])
        # code2 = KL.multiply([w_2d, code2])

        # code2 = Conv2D(c,1,1,padding = 'same',use_bias=False)(code2)
        # code2 = CCattention.criss_cross_attention(code2)
        # code2 = CCattention.criss_cross_attention(code2)
        ...
        code2 = concatenate([code2, code11], axis=3)
